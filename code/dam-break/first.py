...
eqf = uspace.integralalg.edgeintegrator
ebcs,ews = eqf.get_quadrature_points_and_weights()
emeasure = mesh.entity_measure('face',index=index)
face2dof = uspace.face_to_dof()[index]
norm = mesh.face_unit_normal(index=index)
...
rho = uspace.interpolation(rho)
mu = uspace.interpolation(mu)
...
for i in range(0,1):
    ...
    fbb2 = np.einsum('i,ij,ijn,ijk,ijmk,j -> jnm',ws,rho(bcs),uphi,fuu,fgu,cellmeasure)
