...
for i in range(0, nt):
    ...
    area[tag] = 0
    area[tag1] = 1
    areas = space.integralalg.integral(area)
    areaerror = areas - ((np.pi)*0.15*0.15)
    Areaserror.append(areaerror)
...
t = timeline.all_time_levels()
plt.figure()
plt.plot(t, Areaserror,'--', color='g',label='Area error')
