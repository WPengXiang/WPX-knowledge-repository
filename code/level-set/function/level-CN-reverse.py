...
for i in range(0,nt): 
    t1 = t1+dt
    print("t1=",t1)
    E0 = np.einsum('i,ijk,ijm,ij,j ->jkm'\
            ,ws,phi,gphi[...,0],u(bcs)[...,0]*np.cos(t1*np.pi/T),cellmeasure)
    E1 = np.einsum('i,ijk,ijm,ij,j -> jkm'\
            ,ws,phi,gphi[...,1],u(bcs)[...,1]*np.cos(t1*np.pi/T),cellmeasure)
    E = E0+E1
