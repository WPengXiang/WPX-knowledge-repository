...
#组装第二个方程的左端矩阵
B0 = np.einsum('i,ijk,ijm,j -> jkm',ws,pgphi[...,0],pgphi[...,0],cellmeasure)
B1 = np.einsum('i,ijk,ijm,j -> jkm',ws,pgphi[...,1],pgphi[...,1],cellmeasure)
pI = np.broadcast_to(pcell2dof[:,:,None],shape = B0.shape)
pJ = np.broadcast_to(pcell2dof[:,None,:],shape = B0.shape)
B = csr_matrix(((B0+B1).flat,(pI.flat,pJ.flat)),shape=(pgdof,pgdof))
B = pspace.stiff_matrix()
pisBDof = pspace.boundary_dof()
...
for i in range(nt): 
    ...
    b1 = (fb1 - rho*fb2).flatten(order='F')
    b1 = b1+fb4-mu*fb3
    ...
    pisbdf = pspace.boundary_dof()
    pipoint = pspace.interpolation_points()
    pe2 = psolution(pipoint)
    b2[pisbdf] = pe2[pisbdf]
    ...
    errorMatrix[1,i] = pspace.integralalg.L2_error(psolution,p1)
