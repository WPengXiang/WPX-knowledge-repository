...
## Stokes model
from navier_stokes_equation_2d import Poisuille as PDE
...
uspace.set_dirichlet_bc(u1,pde.u_dirichlet)
...
for i in range(0,nt):
    ...
    x[ugdof:-pgdof][u_isBdDof] = uso[u_isBdDof][:,1]
    ...
    uc1 = pde.u_dirichlet(smesh.node)
    NN = smesh.number_of_nodes()
    uc2 = u1[:NN]
    up1 = pde.p_dirichlet(smesh.node)
    up2 = p1[:NN]
   
    errorMatrix[0,i] = uspace.integralalg.L2_error(pde.u_dirichlet,u1)
    errorMatrix[1,i] = pspace.integralalg.error(pde.p_dirichlet,p1)
