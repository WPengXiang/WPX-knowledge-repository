...
class StokesModelData_1:
    """
    [0, 1]^2
    u(x, y) = (-cos(x)*cos(x)*cos(y)*sin(y)/2, cos(x)*sin(x)*cos(y)*cos(y)/2)
    p = sin(x)-sin(y)
    """
    def __init__(self):
        self.box = [0, 1, 0, 1]
    ...
    def velocity(self, p):
        ...
        sin = np.sin
        val = np.zeros(p.shape, dtype=np.float)
        val[..., 0] = -(cos(x)**2*cos(y)*sin(y))/2
        val[..., 1] = (cos(x)*sin(x)*cos(y)**2)/2
        return val
    ...
    def source(self, p):
        ...
        cos = np.cos
        val = np.zeros(p.shape, dtype=np.float)
        val[..., 0] = - 4*sin(y)*cos(y)*cos(x)**2 + cos(y)*sin(y) + cos(x) 
        val[..., 1] =   4*sin(x)*cos(x)*cos(y)**2 - cos(x)*sin(x) - cos(y)
        return val
...
class StokesModelData_2:
    ...
    def source(self, p):
        ...
        cos = np.cos
        val = np.zeros(p.shape, dtype=np.float)
        val[..., 0] = 12*pi**3*sin(pi*x)**2*sin(pi*y)*cos(pi*y) - 4*pi**3*sin(pi*y)*cos(pi*x)**2*cos(pi*y)
        val[..., 1] =-12*pi**3*sin(pi*x)*sin(pi*y)**2*cos(pi*x) + 4*pi**3*sin(pi*x)*cos(pi*x)*cos(pi*y)**2
        return val
...
class StokesModelData_3:
    ...
    def source(self, p):
        ...
        cos = np.cos
        val = np.zeros(p.shape, dtype=np.float)
        val[..., 0] = 12*pi**3*sin(pi*x)**2*sin(pi*y)*cos(pi*y) - 4*pi**3*sin(pi*y)*cos(pi*x)**2*cos(pi*y) + cos(x) 
        val[..., 1] =-12*pi**3*sin(pi*x)*sin(pi*y)**2*cos(pi*x) + 4*pi**3*sin(pi*x)*cos(pi*x)*cos(pi*y)**2 - cos(y)
        return val
