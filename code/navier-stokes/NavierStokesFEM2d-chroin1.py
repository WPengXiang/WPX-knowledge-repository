...
Ndof = np.zeros((nt,))
errorMatrix = np.zeros((len(errorType),nt),dtype=np.float64)
for i in range(nt): 
    # 下一个的时间层 t1
    ...
    b2 = -b2/dt
    sisbdf = pspace.boundary_dof()

    pe2 = psolution(ipoint)
    b2[fisbdf] = pe2[fisbdf]
...
sm.set_array([])
#plt.quiver(x,y,uux,uuy,color=cm(norm(o)))
