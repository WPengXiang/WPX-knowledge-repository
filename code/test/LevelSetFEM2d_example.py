...
'''  
import sys;
sys.path.append('./fealpy/functionspace');
...
M = space.parallel_mass_matrix()
M2 = space.mass_matrix()
#A = space.parallel_stiff_matrix()
#A2 = space.stiff_matrix()
C = space.parallel_convection_matrix(c=u).T 
C1 = space.convection_matrix(c=u)

if method == 'CN':
    A = M + dt/2*C
elif method == 'implicit-Eular':
    A = M + dt*C
elif method == 'Eular':
    A = M 
elif method == 'TVD-3':
...
for i in range(nt):
    ...
    if method == 'CN':
        ...
        b1 = M@phi0 + dt*C@phi0
        phi1 = solver.solve(b1,tol=1e-12)
        b2 = M@phi1 + dt/4*(-3*C*phi0+C@phi1)
        phi2 = solver.solve(b2,tol=1e-12)
        b3 = M@phi2 + dt/12*(-C*phi0-C@phi1+8*C@phi2)
        phi0 = solver.solve(b3,tol=1e-12)
