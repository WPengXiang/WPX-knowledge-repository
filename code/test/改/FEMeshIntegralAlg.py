...
class FEMeshIntegralAlg():
    ...
    def parallel_construct_matrix(self, b0, 
        ...
        index[0] = 0
        index[1:r+1] += 1
        np.cumsum(index, out=index) #index每一块要计算的索引
        ...
        def f(i):
            ...
            for bc, w in zip(bcs, ws): # 对所有积分点进行循环
                ...
                phi1 = basis1(bc, index=s)
             
                if c is None:
                    M += np.einsum('jk..., jm..., j->jkm', \
                            phi0, phi1,w*measure,optimize=True)
                else:
                    if callable(c):
                    ...
                    if isinstance(cval, (int, float)):
                        M += np.einsum('jk..., jm..., j->jkm', phi0, phi1,
                                cval*w*measure, optimize=True)
                    elif isinstance(cval, np.ndarray): 
                        if len(cval.shape) == 1: # (NC)
                            M += np.einsum('j, jk..., jm..., j->jkm',\
                                    cval, phi0, phi1,w*measure, optimize=True)
                        elif len(cval.shape) == 2: # (NC, GD)
                            phi0 = np.einsum('jm, jkm-> jk', cval, phi0)
                            M += np.einsum('jk, jm, j->jkm', phi0, phi1,
                                    w*measure, optimize=True)
                        elif len(cval.shape) == 3: # ( NC, GD, GD)
                            phi0 = np.einsum('jmn, jkn->jkm', cval, phi0)
                            M += np.einsum('jkl, jml, j->jkm', phi0, phi1,
                                    w*measure, optimize=True)
